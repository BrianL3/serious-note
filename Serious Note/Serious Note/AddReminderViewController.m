...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
  ...
  if ([segue.identifier isEqualToString:@"LOCATION_CHOOSE_RECIPIENT"])
  {
    ...
    if([CLLocationManager isMonitoringAvailableForClass:[CLCircularRegion class]])
    {
      ...
      {
        // create a unique string to use as a title by appending the current clock time to the word "AudioVisual"
        NSDate* currentTime = [NSDate date];
        int CurrTime = currentTime.timeIntervalSinceReferenceDate;
        NSString *addOn = [NSString stringWithFormat:@"AudioVisual%d", CurrTime]; // append the time
   ...
   if (self.textField.text.length > 0 || ![self.textField.text containsString:@"AudioVisual"]) // reminder contains text
    {
